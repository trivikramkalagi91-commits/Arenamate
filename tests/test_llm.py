...
from app.services.llm import GeminiPhraser, OfflinePhraser, get_phraser_client
...
def _install_fake_genai(monkeypatch, generate):
    """Infect sys.modules with a fake google.generativeai module."""
...
def test_gemini_phraser_falls_back_on_error(monkeypatch):
    def boom(prompt):
...
def test_gemini_phraser_falls_back_on_empty_text(monkeypatch):
    class Resp:
        text = "   "

    _install_fake_genai(monkeypatch, lambda prompt: Resp())
    ctx = _ctx()
    out = asyncio.run(GeminiPhraser(Settings(gemini_api_key="fake-key")).phrase(ctx, "hi"))
    assert out == compile_response(ctx)
...
def test_factory_returns_gemini_when_key_present(monkeypatch):
    ...
    client = get_phraser_client(Settings(gemini_api_key="fake-key"))
    assert isinstance(client, GeminiPhraser)
